...
all_dup=set()
duplicate_file=open("/gpfs/work4/0/prjs0871/cast22/all_duplicates.txt")
for t in duplicate_file:
    all_dup.add(t)
...
count_dup=0
run_merged_filtered = {}
for q, ranked_list in run_merged.items():
    run_merged_filtered[q] = {}
    for passage_id, score in ranked_list.items():
        docid = passage_id.rsplit('-',1)[0]
        if docid not in all_dup:
            run_merged_filtered[q][passage_id]=score
        else:
            count_dup+=1
print("dedup", count_dup, "elem")

res = {}
for qid in run_merged_filtered:
    if qid not in res:
        res[qid] = {}
    for k,v in sorted(run_merged_filtered[qid].items(),key=lambda item: item[1], reverse=True)[:1000]:
        if k not in res[qid]:
            res[qid][k] = v
        if len(res[qid]) >= 1000:
            break
